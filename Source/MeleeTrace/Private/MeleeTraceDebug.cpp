...
void MeleeTrace::DrawDebugTrace(const UObject* WorldContextObject, const FCollisionShape& CollisionShape,
	const FTransform& StartTransform, const FTransform& EndTransform, EDrawDebugTrace::Type DrawDebugType, bool bHit,
	const TArray<FHitResult>& HitResults, float DrawTime)
{
	...
	{
		DrawDebugLineTraceMulti(World, StartTransform.GetLocation(), EndTransform.GetLocation(), DrawDebugType, bHit, HitResults,
			TraceColor, TraceHitColor, DrawTime);
	}
	else if (CollisionShape.IsSphere())
	{
		DrawDebugSphereTraceMulti(World, StartTransform.GetLocation(), EndTransform.GetLocation(), CollisionShape.GetSphereRadius(),
			DrawDebugType, bHit, HitResults, TraceColor, TraceHitColor, DrawTime);
	}
	else if (CollisionShape.IsCapsule())
	{
		DrawDebugCapsuleTraceMulti(World, StartTransform.GetLocation(), EndTransform.GetLocation(), StartTransform.GetRotation(),
			CollisionShape.GetCapsuleRadius(), CollisionShape.GetCapsuleHalfHeight(), DrawDebugType, bHit, HitResults, TraceColor,
			TraceHitColor, DrawTime);
	}
	else if (CollisionShape.IsBox())
	{
		DrawDebugBoxTraceMulti(World, StartTransform.GetLocation(), EndTransform.GetLocation(), CollisionShape.GetExtent(),
			EndTransform.GetRotation().Rotator(), DrawDebugType, bHit, HitResults, TraceColor, TraceHitColor, DrawTime);
	}
}
...
void MeleeTrace::DrawDebugSphereTraceMulti(const UWorld* World, const FVector& Start, const FVector& End, float Radius,
	EDrawDebugTrace::Type DrawDebugType, bool bHit, const TArray<FHitResult>& HitResults, const FLinearColor& TraceColor,
	const FLinearColor& TraceHitColor, float DrawTime)
{
	...
	if (bHit && HitResults.Last().bBlockingHit) // ※ bBlockingHit : hit occured
	{
		const FVector BlockingHitPoint = HitResults.Last().Location;
		...
		// ※ Draw HitColor Before BlockPoint, Draw NotHitColor After BlockPoint
	}
	else
	{
		DrawDebugSweptSphere(World, Start, End, Radius, TraceHitColor.ToFColor(true), bPersistent, LifeTime);
		// ※ DrawNotHitColor All Point
	}
...
void MeleeTrace::DrawDebugSweptSphere(const UWorld* InWorld, FVector const& Start, FVector const& End, float Radius,
	FColor const& Color, bool bPersistentLines, float LifeTime, uint8 DepthPriority/*=0*/)
{
	// ※ When Not Painting Overlapped Lines, Drawing Swept Sphere Equals with Drawing Capsule

	const FVector TraceVec = End - Start;
	const float Dist = TraceVec.Size();

	const FVector Center = Start + TraceVec * 0.5f;
	const float HalfHeight = (Dist * 0.5f) + Radius;
...
void MeleeTrace::DrawDebugLineTraceMulti(const UWorld* World, const FVector& Start, const FVector& End,
	EDrawDebugTrace::Type DrawDebugType, bool bHit, const TArray<FHitResult>& HitResults, const FLinearColor& TraceColor,
	const FLinearColor& TraceHitColor, float DrawTime)
{
	if (DrawDebugType == EDrawDebugTrace::None) return;
	...
	{
		const FVector BlockingHitPoint = HitResults.Last().ImpactPoint;
...
void MeleeTrace::DrawDebugCapsuleTraceMulti(const UWorld* World, const FVector& Start, const FVector& End, 
	const FQuat& Orientation, float Radius, float HalfHeight, EDrawDebugTrace::Type DrawDebugType, bool bHit, 
	const TArray<FHitResult>& HitResults, const FLinearColor& TraceColor, const FLinearColor& TraceHitColor, float DrawTime)
{
	if (DrawDebugType == EDrawDebugTrace::None) return;
	...
	if (bHit && HitResults.Last().bBlockingHit)
	{
		...
		::DrawDebugCapsule(World, BlockingHitPoint, HalfHeight, Radius, Orientation, TraceColor.ToFColor(true), bPersistent, 
			LifeTime);

		::DrawDebugLine(World, Start, BlockingHitPoint, TraceColor.ToFColor(true), bPersistent, LifeTime);

		::DrawDebugCapsule(World, End, HalfHeight, Radius, Orientation, TraceHitColor.ToFColor(true), bPersistent, LifeTime);
		::DrawDebugLine(World, BlockingHitPoint, End, TraceHitColor.ToFColor(true), bPersistent, LifeTime);
	}
	else
	{
...
{
	// ※ Box Sweep Base Function
	void DrawDebugSweptBox(const UWorld* InWorld, const FVector& Start, const FVector& End, const FRotator& Orientation,
		const FVector& HalfSize, const FColor& Color, bool bPersistentLines, float LifeTime, uint8 DepthPriority = 0)
	{
		const FVector TraceVec = End - Start;
		const FQuat CapsuleRot = Orientation.Quaternion();
		...
		{
			::DrawDebugLine(InWorld, Vertices[VertexIdx], Vertices[VertexIdx] + TraceVec, Color, bPersistentLines, LifeTime,
				DepthPriority);
		}
...
void MeleeTrace::DrawDebugBoxTraceMulti(const UWorld* World, const FVector& Start, const FVector& End, const FVector& HalfSize, 
	const FRotator& Orientation, EDrawDebugTrace::Type DrawDebugType, bool bHit, const TArray<FHitResult>& HitResults, 
	const FLinearColor& TraceColor, const FLinearColor& TraceHitColor, float DrawTime)
{
	if (DrawDebugType == EDrawDebugTrace::None || World == nullptr) return;
	...
	if (bHit && HitResults.Last().bBlockingHit)
	{
		...
		MeleeTrace::DrawDebugSweptBox(World, Start, BlockingHitPoint, Orientation, HalfSize, TraceColor.ToFColor(true), bPersistent, LifeTime);
		MeleeTrace::DrawDebugSweptBox(World, BlockingHitPoint, End, Orientation, HalfSize, TraceHitColor.ToFColor(true), bPersistent, LifeTime);
	}
	else
	{
		MeleeTrace::DrawDebugSweptBox(World, Start, End, Orientation, HalfSize, TraceColor.ToFColor(true), bPersistent, LifeTime);
	}
