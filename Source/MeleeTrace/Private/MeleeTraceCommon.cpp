...
{
	ensureMsgf(!ActiveHashes.Contains(ContextID), TEXT
	("Hash for ContextID = [%u] already exist in the map. Previous hash is going to be overwrite."), ContextID);
...
{
	ensureMsgf(ActiveHashes.Contains(ContextID), TEXT("Attempt to release hash by ContextID = [%u] that is not tracked"),
		ContextID);
	ActiveHashes.Remove(ContextID);

	GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Cyan, FString::Printf(
		TEXT("MeleeTraceCommon.cpp : ReleaseCheck. RestCount is [%d]."), ActiveHashes.Num()));
}
...
uint32 MeleeTrace::CombineHashes(const uint32 LhsHash, const uint32 RhsHash)
{
	...
	return Hash;
#else
	const uint32 Hash = HashCombineFast(LhsHash, RhsHash);// ※ return Will be Used in ActiveHashes's Key. Not Value
	return Hash;
#endif
}
