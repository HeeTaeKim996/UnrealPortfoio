			...
			auto AddressTwoPoint = [&](FVector Start, FVector End) -> void
				{
					...
					const bool bHit = GetWorld()->SweepMultiByChannel(HitResults, Start, End,
						OffsetRotation, TraceChannel, SocketShapeInfo.TraceCollisionShape, CollisionQueryParams);
#ifdef ENABLE_DRAW_DEBUG
					if (bShouldDrawDebug)
					{
						MeleeTrace::DrawDebugTrace(this, SocketShapeInfo.TraceCollisionShape,
							FTransform(OffsetRotation, Start),
							FTransform(OffsetRotation, End), EDrawDebugTrace::Type::ForDuration,
