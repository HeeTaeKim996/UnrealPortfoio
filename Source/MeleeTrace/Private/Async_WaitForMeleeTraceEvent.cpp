...
UAsync_WaitForMeleeTraceEvent* UAsync_WaitForMeleeTraceEvent::WaitForMeleeTraceEventHit(UObject* WorldContextObject,
	AActor* ActorToWatch)
{
	const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, 
		EGetWorldErrorMode::LogAndReturnNull); // ※ Return WorldContextObject's World
	if (!World) return nullptr;

	if (!ActorToWatch)
	{
		UE_LOG(LogMeleeTrace, Error, TEXT("[%s] : Invalid reference for actor to watch passed as parameter"),
			StringCast<TCHAR>(__FUNCTION__).Get());
		return nullptr;
	}
	
	UMeleeTraceComponent* MeleeTraceComponent = ActorToWatch->FindComponentByClass<UMeleeTraceComponent>();
	if (!MeleeTraceComponent)
	{
		UE_LOG(LogMeleeTrace, Error, TEXT("[%s] : Actor to watch doesn't have MeleeTraceComponent Attached"),
			StringCast<TCHAR>(__FUNCTION__).Get());
		return nullptr;
	}

	UAsync_WaitForMeleeTraceEvent* NewAction = NewObject<UAsync_WaitForMeleeTraceEvent>();
...
void UAsync_WaitForMeleeTraceEvent::HandleTraceHit(UMeleeTraceComponent* ThisComponent, AActor* HitActor, 
	...
	uint8 Protocol)
{
	...
	HitInfo.Protocl = Protocol;
	OnHit.Broadcast(HitInfo, TraceHandle);
}

void UAsync_WaitForMeleeTraceEvent::HandleTraceStarted(UMeleeTraceComponent* ThisComponent, FMeleeTraceInstanceHandle TraceHandle)
{
	OnStarted.Broadcast();
}

void UAsync_WaitForMeleeTraceEvent::HandleTraceEnded(UMeleeTraceComponent* ThisComponent, int32 HitCount, FMeleeTraceInstanceHandle TraceHandle)
{
	OnEnded.Broadcast();
}
